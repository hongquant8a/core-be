...
/** Đơn thư đổi trạng thái — `$extraArgs[0]` là trạng thái cũ. */
class PetitionStatusChangedContentBuilder implements ContentBuilder
{
    use BuildZns;
    ...
    {
        return PetitionStatusEnum::tryFrom((string) $status)?->label() ?? (string) $status;
    }

    public function title(User $recipient, Model $notifiable, mixed ...$extraArgs): string
    {
        return 'Đơn thư đổi trạng thái';
    }

    public function shortBody(User $recipient, Model $notifiable, mixed ...$extraArgs): string
    {
        if (! $notifiable instanceof TaskAssignmentPetition) {
            return 'Một đơn thư đã đổi trạng thái.';
        }

        $previous = isset($extraArgs[0]) && is_string($extraArgs[0]) ? $this->statusLabel($extraArgs[0]) : '(không rõ)';

        return sprintf('Đơn thư của %s chuyển từ "%s" sang "%s".',
            $notifiable->sender_name ?: '(không rõ người gửi)',
            $previous,
            $this->statusLabel($notifiable->processing_status));
    }
    ...
    private function toMail(User $recipient, TaskAssignmentPetition $model, mixed ...$extraArgs): ?NotificationPayload
    {
        ...
        $html = view('notifications.petition_status_changed.email', [
            'recipient' => $recipient,
            'model' => $model,
