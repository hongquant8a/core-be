...
class SchedulePublishedContentBuilder implements ContentBuilder
{
    use BuildZns;
    ...
    {
        return 'Lịch công tác mới được ban hành';
    }

    public function shortBody(User $recipient, Model $notifiable, mixed ...$extraArgs): string
    {
        if ($notifiable instanceof Schedule) {
            return "Lịch: {$notifiable->content}";
        }

        return 'Bạn có lịch công tác mới.';
    }
    ...
    public function inAppContext(User $recipient, Model $notifiable, mixed ...$extraArgs): array
    {
        if ($notifiable instanceof Schedule) {
            return [
                ...
                'event' => 'Lịch công tác được ban hành',
            ];
        }
    ...
    private function toSms(User $recipient, Schedule $schedule): ?NotificationPayload
    {
        ...
        $text = "Lich cong tac moi ban hanh: {$schedule->content}. Ngay: {$dateStr}. Xem chi tiet: {$url}";
    ...
    private function toMail(User $recipient, Schedule $schedule): ?NotificationPayload
    {
        ...
        $html = view('notifications.schedule_published.email', [
            'recipient' => $recipient,
            'schedule' => $schedule,
            'url' => $url,
        ])->render();
        ...
        return new NotificationPayload(
            ...
            content: $html,
            subject: "Lịch công tác mới: {$schedule->content}",
        );
    }
    ...
    private function toZalo(User $recipient, Schedule $schedule): ?NotificationPayload
    {
        ...
        $dateStr = $schedule->event_date;
        $url = $this->scheduleFrontendUrl($schedule);
        $text = "Bạn có lịch công tác mới: {$schedule->content} vào ngày {$dateStr}. Xem chi tiết: {$url}";
    ...
    private function toFcm(User $recipient, Schedule $schedule): ?NotificationPayload
    {
        ...
        return new NotificationPayload(
            ...
            content: "Lịch công tác mới: {$schedule->content}",
            subject: 'Lịch công tác mới',
            context: [
                'url' => $this->scheduleFrontendUrl($schedule),
    ...
    private function toTelegram(User $recipient, Schedule $schedule): ?NotificationPayload
    {
        ...
        $dateStr = $schedule->event_date;
        $url = $this->scheduleFrontendUrl($schedule);
        $text = "<b>Lịch công tác mới được ban hành</b>\n\n{$schedule->content}\nNgày: {$dateStr}\nXem chi tiết: {$url}";
