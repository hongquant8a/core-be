...
/**
 * Zalo ZNS channel — DORMANT, giữ làm fallback nếu sau này quay về ZNS template-based qua phone.
 *
 * - Endpoint custom (vd South Telecom relay), Basic auth username/password.
 * - Recipient là `phone` (E.164 normalized 84xxxxxxxxx).
 * - Template-based: cần template duyệt (`zalo_template_id`) + template_data từ payload.context.
 *
 * **Hiện không register trong NotificationServiceProvider** — channel zalo đang dùng OA Message
 * (file ZaloChannel.php). Khi cần swap về ZNS:
 *   - Sửa NotificationServiceProvider::register():
 *     'zalo' => new ZaloZnsChannel($settings)  // thay cho ZaloChannel
 *   - Đảm bảo 6 setting key ZNS đã được điền (zalo_server/username/password/sender/template_id/extra_params)
 *   - ContentBuilders đã sẵn sàng — vẫn build với phone + context giống cũ.
 */
