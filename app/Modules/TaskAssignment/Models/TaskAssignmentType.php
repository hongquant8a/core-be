...
class TaskAssignmentType extends TenantModel
{
    use HasFactory;

    protected static function newFactory()
    {
        return \Database\Factories\Modules\TaskAssignment\Models\TaskAssignmentTypeFactory::new();
    }

    protected $table = 'task_assignment_types';

    protected $fillable = ['name', 'description', 'status', 'organization_id', 'created_by', 'updated_by'];

    protected static function booted()
    {
        static::creating(fn (TaskAssignmentType $model) => $model->created_by = $model->updated_by = auth()->id());
        static::updating(fn (TaskAssignmentType $model) => $model->updated_by = auth()->id());
    }
