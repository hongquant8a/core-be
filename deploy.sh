...
deploy_be() {
  ...
  sudo -u quandh $PHP_BIN artisan migrate:sync-deleted --force --ansi 2>&1 || true
  ...
  for seeder in PermissionSeeder MeetingPermissionSeeder SettingSeeder NotificationEventConfigSeeder OrgSchedulingSettingsSeeder SchedulingModuleSeeder; do
    { sudo -u quandh $PHP_BIN artisan db:seed --class=$seeder --force --ansi 2>&1 | grep -E "DONE|Seeding" | tail -1; } || true
  done

  # Cleanup orphan permissions/settings
  log "  [BE] Cleanup obsolete..."
  sudo -u quandh $PHP_BIN artisan seed:cleanup-obsolete --ansi 2>&1 | tail -2 || true
