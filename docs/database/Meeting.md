...
**Lưu ý:** Cột `allow_host_management` đã được chuyển sang bảng `meetings` (migration 2026-06-18).
...
| has_online_room | boolean | No | false | Có phòng họp online |
| allow_host_management | boolean | No | true | Cho phép chủ tọa/thư ký điều hành (chuyển từ meeting_settings) |
...
| meeting_agenda_id | bigint unsigned | Yes | null | FK → meeting_agendas.id nullOnDelete |
| title | varchar(255) | No | — | |
| is_voting_result_hidden_until_end | boolean | No | false | Ẩn kết quả đến khi chủ đề đóng |
| is_vote_change_allowed | boolean | No | false | Cho phép thay đổi phiếu sau khi đã bỏ |
| calculate_by_total_delegates | boolean | No | true | Tính tỉ lệ theo tổng số đại biểu |
| calculate_by_present_delegates | boolean | No | true | Tính tỉ lệ theo số đại biểu có mặt |
...
### ~~`meeting_reminders`~~ — **ĐÃ XÓA ngày 28/06/2026**
...
`remindable_type = 'App\Modules\Meeting\Models\Meeting'`

Đặc điểm của meeting reminder: hỗ trợ cả `reminder_type = manual` (admin tạo thủ công, có cột `message`) và `reminder_type = scheduled` (tự động theo `notification_schedules`).
...
meeting_attendee_groups ──n-n (pivot)──► meeting_attendees ──┤
                                                         ...
                                                         meetings ──1-n──► meeting_agendas (cây)
                                                             ...
                                                             ├── 1-n ──► reminders [Core] (remindable_type = Meeting)
                                                             └── 1-n ──► meeting_views
```
