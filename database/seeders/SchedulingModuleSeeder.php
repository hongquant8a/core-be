...
class SchedulingModuleSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // 4. Create Schedules
        // Clear old seeds if any to avoid clutter
        // Schedule::truncate();
        ...
        foreach ($schedulesData as $item) {
            ...
            // Add some recipient groups (Skipped for now since group_id references notification_groups)
            // if ($item['content'] == 'Họp giao ban đầu tuần') {
            //     $schedule->recipients()->create([
            //         'group_id' => $groupA->id,
            //         'display_name' => $groupA->name
            //     ]);
            // }
        }
