...
- [ ] Observer chỉ xử lý data integrity, không gửi Notification.
...
│
├─ Side-effect do thay đổi field/model trực tiếp (không qua Service, vd Eloquent::saving)?
...
- Listener: `XuLyKhiXxx` hoặc `SendNotificationOnXxx` — 1 Listener = 1 trách nhiệm.
...
| Ví dụ Danatec | Tự gán `organization_id`, generate `slug`, reindex `VietnameseSort` | `PheDuyetHoSoEvent`, `ChuyenTrangThaiCongViecEvent` |
